...
const ButtonIcons: Record<string, React.FC<React.ComponentProps<"svg">>> = {
  globe: Globe,
  mail: Mail,
  zap: Zap,
  link: ExternalLink,
...
interface SortableBlockItemProps {
  block: Block;
  updateBentoBlock: (blockId: string, updates: Partial<Block>) => void;
...
function SortableBlockItem({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className="bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-2xl sm:rounded-[2.5rem] border border-slate-200 dark:border-white/5 space-y-4 sm:space-y-5 shadow-sm group relative"
    >
...
export default function Editor({
  ...
  const effectiveUsernameStatus =
    !data.username || data.username.trim() === "" ? "idle" : usernameStatus;
  ...
  const handleImageUpload = async (
    ...
    try {
      ...
      if (target?.startsWith("portfolio-")) {
        ...
        const newBlocks = data.content.blocks.map((b) =>
          b.id === target
            ? {
                ...b,
                data: {
  ...
  const addBentoBlock = (type: string) => {
    const defaults: Record<string, BlockData> = {
  ...
  const deleteBentoBlock = (blockId: string) => {
    updateContent({
      blocks: (data.content.blocks ?? []).filter((b) => b.id !== blockId),
    });
  };
    ...
    <div className="space-y-8 sm:space-y-12 pb-32">
      ...
      {activeFeatures.includes("social") && (
        <section className="space-y-4 sm:space-y-6">
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    if (exists) {
                      updateContent({
                        social_links: data.content.social_links?.filter(
                          (s) => s.platform !== p,
                        ),
                      });
                    } else {
          ...
          <div className="space-y-3">
            {data.content.social_links?.map((s) => (
              <div
                key={s.id}
                className="flex items-center gap-3 bg-white dark:bg-slate-900/50 p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-100 dark:border-white/5 shadow-sm"
              >
                <span className="text-[9px] font-black uppercase text-indigo-500 w-14 sm:w-16 shrink-0">
                  {s.platform}
                </span>
                <input
                  value={s.url}
      ...
      {activeFeatures.includes("links") && (
        <section className="space-y-4 sm:space-y-6 pb-20">
          ...
          {data.content.links?.map((link, idx) => (
            <div
              key={link.id}
              className="p-4 sm:p-6 rounded-xl sm:rounded-4xl border bg-white dark:bg-slate-900 border-slate-100 dark:border-white/5 space-y-3 group shadow-sm"
            >
  ...
  const renderBentoControls = () => (
    <div className="space-y-6 sm:space-y-10 pb-32">
      <div className="grid grid-cols-2 gap-2 sm:gap-3">
        {["profile", "link", "image", "social"].map((t) => (
          <button
            ...
            className="p-3 sm:p-4 flex items-center justify-center gap-2 sm:gap-3 bg-white dark:bg-slate-900 border border-slate-100 dark:border-white/5 rounded-xl sm:rounded-2xl text-[10px] font-black uppercase hover:text-indigo-500 hover:border-indigo-500 transition-all shadow-sm"
          >
            <Plus size={14} /> {t}
          </button>
        ))}
      ...
      >
        <SortableContext
          items={(data.content.blocks ?? []).map((b) => b.id)}
  ...
  const renderNexusControls = () => (
    <div className="space-y-8 sm:space-y-12 pb-32">
      <section className="space-y-4 sm:space-y-6 bg-indigo-50/50 dark:bg-indigo-500/5 p-4 sm:p-6 rounded-xl sm:rounded-4xl border border-indigo-100 dark:border-indigo-500/10 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-2 sm:gap-3">
          {["hero", "features", "portfolio", "testimonial", "contact"].map(
            (s) => (
              <button
                key={s}
                ...
                className={`flex items-center justify-between p-3 sm:p-4 rounded-xl sm:rounded-2xl border transition-all text-[10px] font-black uppercase ${
                  data.content.sections_visibility?.[s] !== false
      ...
      <section className="space-y-4 sm:space-y-6">
        ...
        <div className="space-y-3 sm:space-y-4">
          <input
            ...
            onChange={(e) =>
              updateContent({
                hero: { ...data.content.hero, tagline: e.target.value },
              })
            }
          ...
          <input
            ...
            onChange={(e) =>
              updateContent({
                hero: { ...data.content.hero, title: e.target.value },
              })
            }
          ...
          <textarea
            ...
            onChange={(e) =>
              updateContent({
                hero: { ...data.content.hero, subtitle: e.target.value },
              })
            }
      ...
      <section className="space-y-4 sm:space-y-6">
            ...
            onClick={() =>
              updateContent({
                features: [
                  ...
                  { title: "New Feature", desc: "Feature desc..." },
                ],
              })
            }
        ...
        <div className="space-y-3 sm:space-y-4">
          {(data.content.features ?? []).map((f, i) => (
            <div
              key={i}
              className="p-4 sm:p-5 bg-white dark:bg-slate-900 border border-slate-100 dark:border-white/5 rounded-xl sm:rounded-2xl space-y-3 relative group shadow-sm"
            >
              <button
                onClick={() =>
                  updateContent({
                    features: (data.content.features ?? []).filter(
                      (_, idx) => idx !== i,
                    ),
                  })
                }
              ...
              <input
                value={f.title}
                onChange={(e) => {
                  const nf = [...(data.content.features ?? [])];
      ...
      <section className="space-y-4 sm:space-y-6">
        ...
        <div className="space-y-4 sm:space-y-6">
          {(data.content.portfolio ?? []).map((p, i) => (
            <div
              key={i}
            ...
            >
              <button
                onClick={() =>
                  updateContent({
                    portfolio: (data.content.portfolio ?? []).filter(
                      (_, idx) => idx !== i,
                    ),
                  })
                }
      ...
      <section className="space-y-4 sm:space-y-6">
        ...
        <div className="space-y-3 sm:space-y-4">
          <textarea
            value={data.content.testimonial?.text ?? ""}
            onChange={(e) =>
              updateContent({
                testimonial: {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <input
              value={data.content.testimonial?.name ?? ""}
              onChange={(e) =>
                updateContent({
                  testimonial: {
                    ...data.content.testimonial,
            ...
            />
            <input
              value={data.content.testimonial?.role ?? ""}
              onChange={(e) =>
                updateContent({
                  testimonial: {
                    ...data.content.testimonial,
          ...
          <div className="space-y-4 sm:space-y-5">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-4 bg-white dark:bg-slate-900 p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-100 dark:border-white/5 shadow-sm">
                <input
                  type="color"
                  value={data.content.color ?? "#6366f1"}
