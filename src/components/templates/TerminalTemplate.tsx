// src/components/templates/TerminalTemplate.tsx
"use client";
      ...
      <div className="w-full max-w-2xl border border-white/10 rounded-xl overflow-hidden bg-[#0d0d0d] shadow-2xl flex flex-col my-4">
        ...
        <div className="p-4 md:p-8 space-y-5 md:space-y-8 overflow-y-auto">
          <div className="flex items-start gap-4 md:gap-6">
            <div className="shrink-0 relative w-16 h-16 md:w-24 md:h-24">
              {content.avatar_url ? (
                <Image
                  src={content.avatar_url}
                  alt="Avatar"
                  fill
                  className="rounded border border-white/20 object-cover shadow-lg"
                />
              ) : (
                <div
                  className="w-full h-full rounded border border-white/20 flex items-center justify-center text-2xl font-bold"
            ...
            <div className="flex-1 min-w-0">
              <div className="mb-1 flex items-center gap-2">
                ...
                <span className="font-bold text-base md:text-xl truncate block">
                  {content.title}
                </span>
              </div>
                ...
                {content.social_links.map((s: SocialLink) => {
                  ...
                  return (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="flex items-center gap-2 px-3 py-1.5 rounded bg-white/3 hover:bg-white/8 border border-white/5 transition-all active:scale-95"
                    >
                ...
                {content.links.map((l: LinkItem) => {
                  ...
                  return (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="w-full flex items-center justify-between p-3 md:p-4 rounded border border-white/5 bg-white/2 hover:border-white/20 hover:bg-white/5 transition-all group active:scale-[0.99]"
                    >
                      <div className="flex items-center gap-3 truncate">
                        <BIcon
                          ...
                          className="shrink-0"
                        />
                        <span className="text-[11px] md:text-sm font-medium truncate">
                          {l.label}
                        </span>
                      </div>
                      <span
                        className="text-[9px] md:text-[10px] ml-2 shrink-0 opacity-40 md:opacity-0 group-hover:opacity-100 transition-opacity"
                        style={{ color: primaryColor }}
                      >
                        [RUN]
                      </span>
